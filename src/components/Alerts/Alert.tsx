...
const Alert: React.FC<AlertProps> = ({ type, title, message }) => {
  ...
  const selectedStyle = alertStyles[type];

  return (
    <div className={`flex w-full border-l-6 ${selectedStyle.border} ${selectedStyle.bg} px-7 py-8 shadow-md`}>
      <div className={`mr-5 flex h-9 w-9 items-center justify-center rounded-lg ${selectedStyle.iconBg}`}>
        {selectedStyle.icon}
      </div>
      <div className="w-full">
        <h5 className={`mb-3 text-lg font-semibold ${selectedStyle.text}`}>{title}</h5>
